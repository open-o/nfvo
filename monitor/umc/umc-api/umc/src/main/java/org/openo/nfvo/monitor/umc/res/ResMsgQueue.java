 ...
 * 
 */
public class ResMsgQueue extends Thread {

    private static final Logger LOGGER = LoggerFactory.getLogger(ResMsgQueue.class);
    ...
    public ResMsgQueue() {
        poolThreads = new ThreadPoolExecutor(1, maxThreadNum, 2, TimeUnit.MINUTES,
                ...
                new MyThreadFactory("ResMsgQueue"), new DefaultRejectedExecutionHandler());
    }
    ...
    private void dispatchDataRptMsg() {
        while (queue.size() > 0) {
        	String msgData = queue.remove();
            LOGGER.debug("Res msg queue size:" + poolThreads.getQueue().size());
            poolThreads.execute(new ResMsgProcessThread(msgData));
        }
    }
    ...
    private class DefaultRejectedExecutionHandler implements RejectedExecutionHandler {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            LOGGER.info("ResMsgQueue is full!" + executor.getQueue().size());
