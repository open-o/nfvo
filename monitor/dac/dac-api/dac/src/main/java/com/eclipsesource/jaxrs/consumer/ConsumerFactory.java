...
public class ConsumerFactory {
    ...
    /**
     * <p>
     * Creates a consumer object out of a <code>@Path</code> interface. It will create a proxy
     * object of the interface that calls the specified service using the passed in base url.
     * </p>
     *
     ...
     * <p>
     * Creates a consumer object out of a <code>@Path</code> interface. It will create a proxy
     * object of the interface that calls the specified service using the passed in base url. The
     * de/serialization is done using the passed in <code>@Provider</code> objects.
     * </p>
     *
     * @param baseUrl The server url hosting the specified service.
     * @param type The <code>@Path</code> annotated interface class object.
     ...
     * @return a proxy object for the passed in type.
     *
     * @deprecated use {@link ConsumerFactory#createConsumer(String, Configuration, Class)} instead.
     *             Take a look at {@link ClientConfig} to see how a {@link Configuration} can be
     *             created. The custom providers can be registered directly with the
     *             {@link Configurable} methods in {@link ClientConfig}.
     */
    @Deprecated
            ...
            for (Object provider : customProviders) {
                if (provider.getClass().isAnnotationPresent(Provider.class)) {
                    ...
                    throw new IllegalArgumentException(
                            provider.getClass().getName() + " is not annotated with @Provider");
                }
            }
        }
    }
     ...
     * Creates a consumer object out of a <code>@Path</code> interface. It will create a proxy
     * object of the interface that calls the specified service using the passed in base url. The
     * de/serialization is done using the passed in <code>@Provider</code> objects. The passed in
     * {@link Configuration} will be used to create the {@link Client} instances.
     * </p>
     *
     * @param baseUrl The server url hosting the specified service.
    ...
    private static void checkAnnotation(Class<?> type) {
        if (!type.isAnnotationPresent(Path.class)) {
            throw new IllegalArgumentException(
                    type.getName() + " is not a Resource. No @Path Annotation.");
        }
    }
