...
import org.openo.nfvo.vimadapter.service.adapter.inf.IVimAdapter2DriverManager;
 ...
 */
public class VimAdapter2DriverManager implements IVimAdapter2DriverManager {

    private static final Logger LOG = LoggerFactory.getLogger(VimAdapter2DriverManager.class);
    ...
    public JSONObject registerDriver(Map<String, String> paramsMap, JSONObject driverInfo) {
        ...
        if(rsp.getStatus() == Constant.HTTP_CREATED_STATUS_CODE) {
            ...
            resultObj.put("retCode", Constant.HTTP_CREATED_STATUS_CODE);
            return resultObj;
    ...
    public JSONObject unregisterDriver(Map<String, String> paramsMap) {
        ...
        RestfulResponse rsp = VimRestfulUtil.getRemoteResponse(paramsMap, "");
        if(null == rsp) {
        ...
        if(rsp.getStatus() == Constant.HTTP_NOCONTENT_STATUS_CODE) {
            LOG.warn("function=unregisterDriver, msg= status={}, result={}.", rsp.getStatus(), resultCreate);
            resultObj = JSONObject.fromObject(resultCreate);
            resultObj.put("retCode", Constant.HTTP_NOCONTENT_STATUS_CODE);
            return resultObj;
        } else if(rsp.getStatus() == Constant.HTTP_NOTFOUND_STATUS_CODE) {
            LOG.error(
