 ...
 */
public class DiskLimitQuery {

    private static final Logger LOG = LoggerFactory.getLogger(DiskLimitQuery.class);
     ...
     * @since NFVO 0.5
     */
    public DiskLimitQuery(Map<String, String> conMap) {
    ...
    public JSONObject getLimits() {
        ...
        if(null == rsp) {
            LOG.error("function=getDiskLimits,  RestfulResponse is null");
            return restJson;
        }
        String result = rsp.getResponseContent();

        LOG.warn("function=getDiskLimits, result: " + result);
        JSONObject limitObj = JSONObject.fromObject(result);
        if(limitObj.containsKey(Constant.WRAP_QUOTASET)) {
    ...
    private JSONObject getLimitMap(JSONObject limitObj) {
        ...
        restJson.put("limits", limitObj.getJSONObject(Constant.WRAP_QUOTASET));
