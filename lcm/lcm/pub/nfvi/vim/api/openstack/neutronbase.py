...
def get_neutron(funname, auth_info, tenant_name):
    ...
    logger.info("[%s]call neutronclient.Client(auth_url='%s',username='%s',password='%s',tenant_name='%s',insecure=%s,ca_cert='%s')"
        % (funname, url, username, passwd, tenant_name, insecure, cacert))
    return neutronclient.Client(username=username, password=passwd, tenant_name=tenant_name, insecure=insecure, auth_url=url, ca_cert=cacert)
...
def get_neutron_by_tenant_id(funname, auth_info, tenant_id):
    ...
    url = auth_info["url"]
    cacert = auth_info["cacert"]
    logger.info("[%s]call neutronclient.Client(auth_url='%s',username='%s',password='%s',tenant_id='%s',ca_cert='%s')"
